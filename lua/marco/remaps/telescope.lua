...
M.apply = function(builtin)
    ...
    vim.keymap.set('n', '<C-p>', find_fallback, { desc = "Find Git files or files [telescope]" })
    vim.keymap.set('n', '<leader>t', builtin.builtin, { desc = "Telescope telescope [telescope]" })
    vim.keymap.set('n', '<leader>pt', builtin.treesitter, { desc = "Telescope treesitter [telescope]" })
